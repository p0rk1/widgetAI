...
:root{
  ...
  --hi:#FF6A1F;--blue:#4CC9F0;--ok:#3DDC97;--warn:#FFB84D;
...
.gate h1{font-family:'Archivo';font-weight:700;font-size:21px;letter-spacing:-.03em;margin:0 0 7px}
.gate p{color:var(--dim);font-size:13px;line-height:1.6;margin:0 0 22px}
.gate input{width:100%;background:rgba(255,255,255,.025);border:1px solid var(--line);color:var(--chalk);padding:12px 14px;font-family:'JetBrains Mono';font-size:13px;transition:border-color .25s,background .25s}
.gate input:focus{outline:none;border-color:var(--hi);background:rgba(255,106,31,.04)}
.gate button{width:100%;margin-top:11px;background:var(--hi);color:#0A0D11;border:none;padding:12px;font-family:'Archivo';font-weight:700;font-size:12px;letter-spacing:.07em;text-transform:uppercase;cursor:pointer;transition:transform .18s var(--sp),box-shadow .25s}
.gate button:hover{transform:translateY(-2px);box-shadow:0 8px 26px rgba(255,106,31,.3)}
.gate .err{color:var(--warn);font-size:12px;margin-top:12px;display:none;font-family:'JetBrains Mono'}
...
.q .meta{font-family:'JetBrains Mono';font-size:9.5px;color:var(--dim);margin-top:4px;letter-spacing:.05em}
...
.tl .tb{width:100%;background:linear-gradient(180deg,var(--hi),rgba(255,106,31,.25));height:0;transition:height .8s var(--sp);min-height:2px}
